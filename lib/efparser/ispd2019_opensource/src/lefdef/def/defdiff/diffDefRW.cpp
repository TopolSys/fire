...
int cls(defrCallbackType_e c, void* cl, defiUserData ud) {
  ...
  case defrScanchainCbkType :
         ...
         if (sc->hasStart()) {
             sc->start(&a1, &b1);
             fprintf(fout, " START %s %s", a1, b1);
         }
         if (sc->hasStop()) {
             sc->stop(&a1, &b1);
             fprintf(fout, " STOP %s %s", a1, b1);
         }
  ...
  case defrTimingDisableCbkType :
         ...
         if (td->hasFromTo())
             fprintf(fout, "TIMINGDISABLE FROMPIN %s %s ", td->fromPin(), td->toPin());
         if (td->hasThru())
         ...
         if (td->hasMacroThru())
             fprintf(fout, " MACRO %s THRUPIN %s %s ", td->macroName(), td->thruInst(), td->thruPin());
         fprintf(fout, "\n");
         break;

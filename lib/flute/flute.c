...
int memLUT[D+1][MGROUP];	// 0=no need clean up, 1=need memory free
...
void readLUT();
void endLUT();	// free memory of LUT
...
void readStringLUT()
{
    ...
    fpwv=fmemopen(POWV9, strlen(POWV9), "r");
    if (fpwv == NULL) {
...
#if ROUTING==1
    fprt=fmemopen(POST9, strlen(POST9), "r");
    if (fprt == NULL) {
        printf("Error in opening %s\n", POSTFILE);
        exit(1);
    }
...
void endLUT(){
	...
	//printf("%d different LUT entry\n", nFree );
    extern void end_param();
    end_param();
}
